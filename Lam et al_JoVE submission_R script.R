...
#colony buoyant weight comparison
b_weight.anova <- aov(b_weight~treatment, data=data_RO)
summary(b_weight.anova)

mean(data_RO$b_weight)
#49.27542
sd(data_RO$b_weight)
#13.22973
...
RO_Vplot  + theme(text=element_text(size=14,  family="serif")) +
...
#contrast plots
plot(RO.mm_posthoc, by = "feeding_treatment", comparisons=TRUE)
plot(RO.mm_posthoc, by = "temp_treatment", comparisons=TRUE)
emmip(RO.mm, ~temp_treatment | feeding_treatment, CIs = TRUE) 
emmip(RO.mm, ~feeding_treatment | temp_treatment, CIs = TRUE)
